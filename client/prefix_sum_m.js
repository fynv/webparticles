...
export function PrefixSum(commandEncoder, psystem)
{
    ...
    for (let i=0; i<psystem.dCellCountBufs.length; i++)
    {
        if (i<psystem.dCellCountBufs.length - 1)
        {
            ...
            let num_groups = Math.floor((psystem.dCellCountBufSizes[i] + workgroup_size_2x - 1)/workgroup_size_2x); 
    ...
    for (let i = psystem.dCellCountBufs.length-2; i>=0; i--)
    {
        ...
        let num_groups = Math.floor((psystem.dCellCountBufSizes[i] + workgroup_size - 1)/workgroup_size) - 2; 
        passEncoder.setPipeline(pipeline);
